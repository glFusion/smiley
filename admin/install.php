...
require_once $_CONF['path'].'/plugins/smiley/autoinstall.php';

USES_lib_install();

if (!SEC_inGroup('Root')) {
    // Someone is trying to illegally access this page
    COM_errorLog("Someone has tried to illegally access the Smiley install/uninstall page.  User id: {$_USER['uid']}, Username: {$_USER['username']}, IP: $REMOTE_ADDR",1);
...
if (SEC_checkToken()) {
    ...
    if ($action == 'install') {
    	...
    	if (plugin_uninstall_smiley('installed')) {
    		/**
    		* Redirects to the plugin editor
    		*/
